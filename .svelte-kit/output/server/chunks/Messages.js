import { c as create_ssr_component, b as subscribe, l as createEventDispatcher, p as getContext, g as escape, a as add_attribute, e as each, v as validate_component, o as onDestroy, u as null_to_empty } from "./ssr.js";
import { t as tick } from "./scheduler.js";
import { v4 } from "uuid";
import { z as tags, m as models, l as WEBUI_API_BASE_URL, o as mobile, d as chatId, a as settings, c as config, T as TTSWorker, u as user, t as temporaryChatEnabled, g as WEBUI_BASE_URL, b as currentChatPage, y as chats } from "./index3.js";
...
import { a as createMessagesList, h as formatDate, s as sanitizeResponseContent } from "./index5.js";
...
const ResponseMessage = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${validate_component(ConfirmDialog, "DeleteConfirmDialog").$$render(
      ...
      {}
    )} <div class="${"flex w-full message-" + escape(message.id, true) + " svelte-1u5gq5j"}" id="${"message-" + escape(message.id, true)}"${add_attribute("dir", $settings.chatDirection, 0)}><div class="${escape(
      null_to_empty(`shrink-0 ${($settings?.chatDirection ?? "LTR") === "LTR" ? "mr-3" : "ml-3"}`),
      true
    ) + " svelte-1u5gq5j"}">${validate_component(ProfileImage, "ProfileImage").$$render(
      $$result,
      {
      ...
      default: () => {
        return `${validate_component(Tooltip, "Tooltip").$$render(
          ...
          {
            default: () => {
              return `<span class="line-clamp-1">${escape(model?.name ?? message.model)}</span>`;
            }
          }
    ...
    })() : ``} ${`<div class="w-full flex flex-col relative" id="response-content-container">${message.content === "" && !message.error ? `${validate_component(Skeleton, "Skeleton").$$render($$result, {}, {}, {})}` : `${message.content && message.error !== true ? `  ${validate_component(ContentRenderer, "ContentRenderer").$$render(
      $$result,
      {
        id: message.id,
        history,
