...
const hclHighlight = highlight.styleTags({
    "for in if else endfor endif": highlight.tags.controlKeyword,
    "Attribute/Identifier": highlight.tags.definition(highlight.tags.propertyName),
    "ForIntro/Identifier": highlight.tags.definition(highlight.tags.propertyName),
    "Block/Identifier": highlight.tags.definitionKeyword,
    "FunctionCall/Identifier": highlight.tags.function(highlight.tags.variableName),
    "Attribute/Expression": highlight.tags.attributeValue,
    "VariableExpr/Identifier": highlight.tags.variableName,
    "GetAttr/Identifier": highlight.tags.variableName,
    Identifier: highlight.tags.name,
    "LiteralValue/...": highlight.tags.literal,
    "StringLit/... TemplateLiteral": highlight.tags.string,
    NumericLit: highlight.tags.number,
    BoolLit: highlight.tags.bool,
    NullLit: highlight.tags.null,
    ...
    HeredocIdentifier: highlight.tags.special(highlight.tags.controlKeyword),
    HeredocStart: highlight.tags.special(highlight.tags.brace),
    "TemplateInterpolationStart TemplateInterpolationEnd": highlight.tags.special(highlight.tags.brace),
    "TemplateDirectiveStart TemplateDirectiveEnd": highlight.tags.special(highlight.tags.brace),
    StripMarker: highlight.tags.special(highlight.tags.brace),
...
const trackTemplate = new lr.ContextTracker({
  start: topCtx,
...
const scanQuotedTemplateStart = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const scanQuotedTemplateEnd = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const scanTemplateLiteralChunk = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const scanTemplateInterpolationStart = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const scanTemplateInterpolationEnd = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const scanTemplateDirectiveStart = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const scanTemplateDirectiveEnd = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const scanHeredocIdentifier = new lr.ExternalTokenizer(
  (input, stack) => {
    if (
...
const hclLanguage = language.LRLanguage.define({
    parser: parser.configure({
        props: [
            ...
            language.foldNodeProp.add({
                "Object Tuple TemplateFor TemplateIf": language.foldInside,
                BlockComment: (tree) => {
...
function hcl() {
    return new language.LanguageSupport(hclLanguage);
}

exports.hcl = hcl;
exports.hclLanguage = hclLanguage;
